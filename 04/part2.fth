...
: mark-number-on-board ( number board -- won? )
  ...
  dup -1 =
    ...
    then
;
: is-board-winner? ( board -- won? )
  false
  5 0 do
    over board.row-free# i + c@ =0 or
    over board.col-free# i + c@ =0 or
    dup if leave then
  loop
;

: mark-number ( number -- winner? )
  boards vec>size 1- ( number board-index )
  begin dup -1 >
  while
    2dup boards vec[] mark-number-on-board if
      boards vec>size 1 =
        if 2drop -1 exit
        else dup boards remove-vec-item
        then
    then
    1-
  repeat
