...
create exprsbuf buf allot
1024 exprsbuf init-buf
create expressions vec allot
2 cells expressions init-vec

: append-new-expression ( c-addr u -- )
  dup >r \ track expression length for later
  exprsbuf buf>size >r \ track current buf size for later
  dup exprsbuf reserve-buf-space swap move \ push expr into buf
  r> r> expressions append-vec-item 2! \ and store its index and size
;

: expressions[]@ ( i -- c-addr u )
  expressions vec[] 2@
  swap exprsbuf buf[] swap
;
...
: clear-expression ( -- ) expr clear-buf ;
...
: compute-magnitude-of ( i j -- u )
  swap
  clear-expression
  PAIR_START push-term
...
: max-magnitude ( -- u )
  0
  expressions vec>size 0 do
    expressions vec>size 0 do
      i j <> if
        i j compute-magnitude-of max
      then
    loop
  loop
;
...
run
expr destroy-buf
exprsbuf destroy-buf
expressions destroy-vec
bye