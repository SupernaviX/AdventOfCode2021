...
: room[] ( i room state -- address ) s.rooms -rot 2* 2* + + ;
: room[]-front ( room state -- address ) 3 -rot room[] ;
: room-bounds ( -- end start ) 4 0 ;
: reversed ( end start -- end start ) swap 1- ;
...
: state. ( state -- )
  ." #############" cr
    ...
    ." #" cr
  ." ###" room-bounds do dup 3 i rot room[] c@ tile. ." #" loop ." ##" cr
  room-bounds reversed 1- do
    space space ." #" room-bounds do dup j i rot room[] c@ tile. ." #" loop cr
  -1 +loop
  space space ." #########" cr
  drop
;
...
: state= ( s1 s2 -- ? )
  ...
  2dup s.rooms cell + 2@ rot s.rooms cell + 2@ d= -rot
  s.hallways 2@ rot s.hallways 2@ d= and and
;
: state-hash ( state -- hash )
  dup s.rooms 2@ hash+
  over s.rooms cell + 2@ dhash+
...
: current-room[] ( i room -- val ) current-state room[] c@ ;
...
\  next-line 2drop \ skip empty hallway
  ...
  hallway-bounds reversed do
    i current-state hallway[] c!
  -1 +loop
  next-line parse-room-amphipods
  room-bounds reversed do
    3 i current-state room[] c!
  -1 +loop
  s" ###D#C#B#A#" parse-room-amphipods
  room-bounds reversed do
    2 i current-state room[] c!
  -1 +loop
  s" ###D#B#A#C#" parse-room-amphipods
  room-bounds reversed do
    1 i current-state room[] c!
  -1 +loop
  next-line parse-room-amphipods
  room-bounds reversed do
    0 i current-state room[] c!
  -1 +loop
...
variable scoochroom
variable scoochstate
: scoochroom[] ( i -- address ) scoochroom @ scoochstate @ room[] ;

: must-scooch-out? ( i -- ? )
  scoochroom[] c@ ( value )
  dup <>0
  swap scoochroom @ 1+ <> and
;
: farthest-back-unscooched ( -- i )
  0 must-scooch-out? if 0 exit then
  1 must-scooch-out? if 1 exit then
  2 must-scooch-out? if 2 exit then
  3
;
: scooch-out ( i -- )
  dup scoochroom[] c@ ?dup =0 if drop exit then
  >r
  dup scoochroom[] 0 swap c!
  1+ scoochroom[] r@ swap c!
  r> amphipod-energy scoochstate @ s.cost +!
;
: try-scooch-out ( room state -- )
  2dup room[]-front c@ if
    2drop exit \ don't scooch out if someone is in front
  then
  scoochstate ! scoochroom !  
  farthest-back-unscooched dup 3 = if drop exit then
  2 do
    i scooch-out
  -1 +loop
;
...
: try-scooch-in ( room state -- )
  scoochstate ! scoochroom !
  0 2 do
    i scoochroom[] c@ if leave then \ stop scoochin when we hit someone
    i 1+ scooch-in
  -1 +loop
;
...
: generate-successors ( -- )
  ...
  current-state s.cost @ max-state @ > if
    ...
    current-state s.cost @ 127 and =0 if
